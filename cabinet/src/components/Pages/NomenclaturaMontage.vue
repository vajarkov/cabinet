    ...
    export default {
        ...
        methods: {
            ...
            async getNomenclatura(){
                ...
                if(this.$store.state.auth.user.session.staff.full_access === 1){
                    if (this.staff.id == 0){
                        this.$store.dispatch('montage/WriteOff', {end:endDate, start:startDate ,key:user.session.client.key}).then(
                        (nomenclatura) => {
                        ...
                        (error) => {
                         ...
                         this.$store.dispatch('montage/WriteOffStaff', {end:endDate, start:startDate, key:user.session.client.key, staff: this.staff.id}).then(
                        (nomenclatura) => {
                   ...
                   if (this.staff.id == 0){
                            this.$store.dispatch('montage/WriteOffBranch', {end:endDate, start:startDate, key:user.session.client.key, branch: this.$store.state.auth.user.session.branch.id}).then(
                            (nomenclatura) => {
                                this.nomenclaturas = nomenclatura.data
