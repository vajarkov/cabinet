<template>
        ...
        <div class="card mx-0 py-0 px-0 my-0">
                    ...
                    <div class="card-header d-flex">
                        ...
                        <select v-model="staff" class="form-select form-select-sm mx-2" aria-label=".form-select-sm example" 
                        ...
                        <button class="mx-5 w-25 h-10 text-light bg-primary"  style="align:center" @click="getNomenclatura()">Получить данные</button>
                    </div>
                    <div class="card-body px-0 py-0 my-0">
                        <table class="table table-hover table-bordered"> 
                                ...
                                <tbody v-for="nomenclatura in nomenclaturas" :key="nomenclatura.id">
                                    <tr>
                                        <td>{{ nomenclatura.stamp}}</td>
                                        <td>{{ nomenclatura.staff}}</td>
                                        <td>{{ nomenclatura.address}}</td>
                                        <td>{{ nomenclatura.name }}</td>
                                        <th scope="row">{{ nomenclatura.amount }}</th>
    ...
    export default {
        ...
        methods: {
            ...
            async getNomenclatura(){
                ...
                if(this.$store.state.auth.user.session.staff.full_access === 1){
                    if (this.staff.id == 0){
                        this.$store.dispatch('reports/WriteOff', {end:endDate, start:startDate ,key:user.session.client.key}).then(
                        (nomenclatura) => {
                            
                            this.nomenclaturas = nomenclatura.data
                        ...
                        (error) => {
                         ...
                         this.$store.dispatch('reports/WriteOffStaff', {end:endDate, start:startDate, key:user.session.client.key, staff: this.staff.id}).then(
                        (nomenclatura) => {
                   ...
                   if (this.staff.id == 0){
                            this.$store.dispatch('reports/WriteOffBranch', {end:endDate, start:startDate, key:user.session.client.key, branch: this.$store.state.auth.user.session.branch.id}).then(
                            (nomenclatura) => {
                                this.nomenclaturas = nomenclatura.data
        ...
        mounted() {
            document.title = "КСУ Списание номенклатуры"
