    ...
    <div>
        <div class="card mx-0 py-0 px-0 my-0">
                    ...
                    <div class="card-header d-flex">
                        ...
                        <input class="mx-2" style="width:50px;height:30px;align:center;" v-model.number="percent" type="number" />
                        <span v-show="Object.keys(porches).length!==0" @click="exportExcel()"><font-awesome-icon  icon="file-excel" /></span>
                        <button class="mx-2 w-145 text-light" style="background:#276595;align:center;position: absolute; right: 0;" @click="getPercent()">Получить данные</button>
                    </div>
                    <div class="card-body px-0 py-0 my-0">
                        <table class="table table-hover table-bordered"> 
                                ...
                                <tbody v-for="porch in porches" :key="porch.id">
                                    ...
                                    <tr v-show="porch.percent <= percent">
    ...
    export default {
        ...
        methods: {
            ...
            async getPercent(){
                ...
                this.loading = true
                var user = this.$store.state.auth.user
                this.$store.dispatch('reports/Percent', {key:user.session.client.key, branch: this.branch.id}).then(
                    (porches) => {
            ...
            exportExcel () {
                ...
                saveExcel({
                    ...
                    columns: [
                        ...
                        { field: 'count', title:'Кол-во абонентов' },
        ...
        mounted() {
            document.title = "КСУ Количество абонентов в подъезде, %"
