<template>
        ...
        <div class="card mx-0 py-0 px-0 my-0">
                    ...
                    <div class="card-header d-flex">
                        ...
                        <span v-show="Object.keys(invoices).length!==0" @click="exportExcel()"><font-awesome-icon  icon="file-excel" /></span>
                       
                        
                        <button class="mx-5 w-25 h-10 text-light"  style="background:#276595;align:center;position: absolute; right: 0;height:30px;" 
                            @click="getInvoice()">Получить данные</button>
                    </div>
                    <div class="card-body px-0 py-0 my-0">
                        <table class="table table-hover table-bordered"> 
                                ...
                                <tbody v-for="invoice in invoices" :key="invoice.id">
                                    <tr>
                                        <td>{{ invoice.datedoc}}</td>
                                        <td>{{ invoice.numdoc}}</td>
                                        <td>{{ invoice.staff}}</td>
                                        <td>{{ invoice.name }}</td>
    ...
    export default {
        ...
        methods: {
            ...
            async getInvoice(){
                ...
                if(this.$store.state.auth.user.session.staff.full_access === 1){
                    if (this.staff.id == 0){
                        this.$store.dispatch('reports/Invoice', {end:endDate, start:startDate ,key:user.session.client.key}).then(
                        (invoice) => {
                            //console.log(invoice)
                        ...
                        (error) => {
                         ...
                         this.$store.dispatch('reports/InvoiceStaff', {end:endDate, start:startDate, key:user.session.client.key, staff: this.staff.id}).then(
                        (invoice) => {
                            //console.log(invoice)
                            this.invoices = invoice.invoice
                   ...
                   if (this.staff.id == 0){
                            this.$store.dispatch('reports/InvoiceBranch', {end:endDate, start:startDate, key:user.session.client.key, branch: this.$store.state.auth.user.session.branch.id}).then(
                            (invoice) => {
                                //console.log(invoice)
                                this.invoices = invoice.invoice
